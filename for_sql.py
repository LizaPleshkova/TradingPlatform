...
from work_shift.models import Position, WorkShift, Plan
...
# для заполнения бд - WS -100 Pos-100
def init_pos():
    for i in range(100):
...
def upt_pos():
    ...
    # random.uniform(65, 164)
    # for i in range(20,120):
    for i in ws:
        print(i)
        r = random.randint(1, 98)
        i.assigned_positions.add(pos[r])
        i.assigned_positions.add(pos[r - 1])
        i.assigned_positions.add(pos[r + 1])
        # i.save(updating_fields=[workshifts])
        i.save()


def init_ws():
    ''' created 500 WorkShift'''
